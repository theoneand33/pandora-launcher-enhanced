...
* **`sort_unstable_by` vs `sort_by`**: Unstable sort does not preserve insertion order for equal keys. For `content_summary.id` + filename ordering, keys are unique. For `SystemTime` ordering (backend.rs:363, syncing.rs:179), equal timestamps are broken by path comparison for deterministic order. No UI relies on stability.
