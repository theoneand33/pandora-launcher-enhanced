...
* Relay mode (separate website): launcher does `PUT /p2p/<token>` to `p2p_relay_url` (Coolify). The relay stores the zip 30 min. Link becomes `https://relay.example.com/p2p/<token>`. The relay is the only URL. No binary touches GitHub.
...
## Domain use — separate website (Coolify)

The domain website lives outside the launcher repo.
...
* `p2p_relay_url` set → launcher uploads via `PUT`, shows `https://relay.../p2p/<token>`. On upload failure it falls back to LAN links.
...
3. `crates/schema/src/backend_config.rs`: `p2p_relay_url`.
4. `~/pandora-sync/` (separate git repo): `index.html` + `README.md` + `relay/` (Coolify). Deploy `relay/` to Coolify.
